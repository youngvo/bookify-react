...
(function () {
  ...
  _500pxSDK = function () {
    ...
    this.init = function (options) {
      let init_1_string = 'init: Already initialized';
      if (this.sdk_key) {
        throw init_1_string;
      }
      let init_2_string = 'init: You must specify an sdk key';
      if (!options || !options.sdk_key) {
        throw init_2_string;
      }
      let init_3_string = 'init: Could not find the body element, make sure the document is loaded before calling init';
      if (!document.body) {
        throw init_3_string;
      }
    ...
    //    });
    this.api = function () {
      let api_1_string = "api: SDK not initialized. Use _500px.init() first."
      if (!this.sdk_key) {
        throw api_1_string;
      }
      ...
      url = args.shift();
      let api_2_string = 'api: You must specify an end point';
      ...
      tag_src += '?';
      tag_src += _object_to_params(data);
      tag.src = tag_src;
    ...
    this.login = function (callback) {
      let login_string = "login: SDK not initialized. Use _500px.init() first.";
      if (!this.sdk_key) {
        throw login_string;
      }
      ...
      left_offset = (window.screen.width / 2) - (840 / 2);
      top_offset = (window.screen.height / 2) - (480 / 2);
    ...
    this.authorize = function (callback) {
      let authorize_string = "authorize: SDK not initialized. Use _500px.init() first.";
      if (!this.sdk_key) {
        throw authorize_string;
      }
      this.login(callback);
    };
    ...
    this.ensureAuthorization = function (callback) {
      let ensureAuthorization_string = "ensureAuthorization: SDK not initialized. Use _500px.init() first.";
      if (!this.sdk_key) {
        throw ensureAuthorization_string;
      }
    ...
    this.getAuthorizationStatus = function (callback) {
      let getAuthorizationStatus_string = 'getAuthorizationStatus: SDK not initialized. Use _500px.init() first.';
      if (!this.sdk_key) {
        throw getAuthorizationStatus_string;
      }
    ...
    this.on = function (event_name, callback) {
      ...
      let on_string = 'on: Callback is not a function';
    ...
    this.logout = function (callback) {
      let logout_1_string = "logout: SDK not initialized. Use _500px.init() first.";
      if (!this.sdk_key) {
        throw logout_1_string;
      }
      let logout_2_string = "logout: User is not logged in";
      if (!oauth_token) {
        throw logout_2_string;
      }
      ...
      if (navigator.userAgent.match(/MSIE/)) {
        left_offset = (window.screen.width / 2) - (1240 / 2);
        top_offset = (window.screen.height / 2) - (480 / 2);
    ...
    function _encode_param_name(name, root) {
      if (root) {
    ...
    function _object_to_params(object, root) {
        ...
        if (object.hasOwnProperty(property)) {
          ...
          if (value instanceof Array) {
            for (i = 0; i < value.length; i++) {
              ...
              string_parts.push(_encode_param_name(property, root) + '%5B%5D=' + encoded_value);
            }
          } else if (typeof value === 'object') {
            string_parts.push(this._object_to_params(value, _encode_param_name(property, root)));
          } else {
            string_parts.push(_encode_param_name(property, root) + '=' + encodeURIComponent(value));
          }
        }
      }
