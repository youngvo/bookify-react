...
* `src/actions` all action types are defined here.
* `src/actions` all app-assets should be put here, for example: locale, images, etc...
* `src/components` all UI/UX are organized here.
* `src/components` contains all app-configurations.
* `src/libs` all third-party libraries must be placed inside this folder.
* `src/styles` contains all common styles, such as, background, page background color.
* `src/utils` all utility functions are put here.
